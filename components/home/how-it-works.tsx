import { BrainCircuit, FileText, MoveRight } from "lucide-react";
...
const steps: Steps[] = [
  {
    ...
    label: "Upload PDF",
    discription: "Simply drag and drop your PDF document or click to upload",
  },
  {
  ...
  },
  {
    icon: <FileText size={64} strokeWidth={1.5} />,
    label: "Get Summary",
    discription: "Receive a clear, concise summary of your document",
  },
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto relative">
          {steps.map((step, idx) => (
            <div className="relative flex items-stretch" key={idx}>
              <StepItem {...step} />
              {idx < steps.length - 1 && (
                <div className="hidden md:block absolute   top-1/2 -right-4 trsnsform -translate-y-1/2 z-10  ">
                  <MoveRight
                    size={32}
                    strokeWidth={1}
...
function StepItem({ icon, label, discription }: Steps) {
  return (
    <div className="relative p-6 rounded-2xl  bg-white/5 backdrop-blur-xs border border-white/10 hover:border-rose-500 transition-colors group w-full">
      <div className="flex flex-col gap-4 h-full">
        ...
        <div className="flex flex-col  flex-1 gap-1 justify-between  ">
          ...
          <p className="text-center text-gray-600 text-sm">{discription}</p>
        </div>
      </div>
    </div>
  );
